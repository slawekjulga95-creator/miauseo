...
export default function StickyContactWidget() {
  ...
  const inp = (err: boolean) =>
    `w-full px-4 py-2.5 rounded-xl bg-white/15 placeholder-white/50 text-white text-sm outline-none focus:ring-2 focus:ring-white/40 transition ${err ? "ring-2 ring-white" : ""}`;
  ...
  return (
    ...
    >
      <button onClick={() => setClosed(true)} aria-label="Zamknij"
        ...
        {submitted ? (
          <div className="p-7 flex flex-col items-center text-center h-full justify-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/75 text-sm leading-snug">Odezwiemy się wkrótce.</p>
            </div>
          </div>
        ) : (
          <div className="p-5">
            <form onSubmit={handleSubmit} noValidate className="space-y-2.5">
              <div>
                ...
                {errors.name && <p className="text-white/80 text-xs mt-1">Podaj swoje imię</p>}
              </div>
              <div>
                <input type="tel" placeholder="Telefon *" value={form.phone}
                  onChange={(e) => { setForm((f) => ({ ...f, phone: e.target.value })); if (errors.phone) setErrors((er) => ({ ...er, phone: false })); }}
                  className={inp(errors.phone)} />
                {errors.phone && <p className="text-white/80 text-xs mt-1">Podaj numer telefonu</p>}
              </div>
              ...
              <div>
                ...
                {errors.email && <p className="text-white/80 text-xs mt-1">Podaj e-mail</p>}
              </div>
              <textarea placeholder="Jak możemy pomóc?" rows={2} value={form.message}
            ...
            <p className="text-white/40 text-[10px] text-center mt-3 leading-snug">Bez spamu. Bez zobowiązań.</p>
          </div>
        )}
      </div>
    </div>
