...
type Lesson = { id: string; title: string; order: number; video_url: string; duration_min: number; is_free_preview: boolean; section_id: string | null };
...
export default function LessonPlayer({ course, lessons, sections, currentLesson, completedIds, userId }: Props) {
    ...
    <div className="fixed inset-0 z-[100] bg-ink text-white flex flex-col overflow-hidden">
      ...
      <header className="border-b border-white/10 px-6 py-4 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="text-xs text-white/50">{completed.size}/{lessons.length}</div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <div className="aspect-video bg-black w-full shrink-0">
            {active.video_url ? (
              <iframe src={active.video_url} className="w-full h-full" allowFullScreen allow="autoplay; fullscreen" />
            ) : (
          ...
          <div className="p-6 overflow-y-auto">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <p className="text-xs font-bold tracking-widest uppercase text-brand mb-2">Lekcja {active.order}</p>
                <h2 className="text-xl font-bold text-white">{active.title}</h2>
                {active.duration_min > 0 && <p className="text-sm text-white/50 mt-1">{active.duration_min} min</p>}
              </div>
        ...
        {/* Sidebar */}
        <aside className="w-80 border-l border-white/10 flex flex-col shrink-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {/* Lekcje bez sekcji */}
            {lessonsWithoutSection.map((lesson) => (
              <LessonItem key={lesson.id} lesson={lesson} active={active} completed={completed} onClick={() => setActive(lesson)} />
            ))}
            ...
            {sections.map((section) => {
              ...
              return (
                <div key={section.id}>
                  <div className="px-5 py-3 bg-white/5 border-b border-t border-white/10">
...
function LessonItem({ lesson, active, completed, onClick }: {
  lesson: Lesson; active: Lesson; completed: Set<string>; onClick: () => void;
}) {
  return (
    <button onClick={onClick}
      className={`w-full text-left px-5 py-4 border-b border-white/5 flex items-start gap-3 transition-colors hover:bg-white/5 ${active.id === lesson.id ? "bg-white/10" : ""}`}>
      <div className={`shrink-0 w-5 h-5 rounded-full border flex items-center justify-center mt-0.5 ${
        completed.has(lesson.id) ? "bg-green-500 border-green-500" : active.id === lesson.id ? "border-brand" : "border-white/20"
      }`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {lesson.duration_min > 0 && <p className="text-xs text-white/30 mt-0.5">{lesson.duration_min} min</p>}
      </div>
    </button>
  );
}
